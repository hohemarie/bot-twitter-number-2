...
while count<5 do
	...
	m+="@"
	m+=p
	m+=" merci pour votre fonctionnement central dans le democratie #bonjour_monde"
	liste<<m
	count+=1
end
#liste.each do |message|

#quelques cles q:qui qppellent lescles API de ton fichier .env
	#require 'dotenv'
	#require 'twitter'
	#Dotenv.load
...
client = Twitter::REST::Client.new do |config|
	#end
#client.update("message tweeettweete tweet tweet tweet")
	#client.follow(pseu)
end
#client.user_timeline(pseudos[0])
#i=client.search("#bonjour_monde", result_type: "recent").take(20)
#i.each do |tweet|
#  client.follow(tweet.user.text)
#end
